classdef controlWindow < handle
    ...
    methods
            ...
            switch src.Tag
                ...
                case 'videoLoad'
                    ...
                    if isempty(obj.videoShowObj) % If no video show exists, create one. Only one will be used for any number of videos displayed.
                        ...
                        obj.h.eVideoOffset.Enable = 'on';
                        [obj.h.bVideoOffset.Enable] = deal('on');
                ...
                case 'videoLoad4'
                    % First ask user for the file path and names. If the user changes their mind during the uigetfile and selects nothing,
                    % getFilepn returns empty and nothing happens so no new videoObj is created.
                    filepn = controlWindow.getFilepn('Select video files', 'on', '\*-4.*', 'video');
                    if isempty(filepn)
                        return
                    end
                    if length(filepn) ~= length(obj.signalObj.filepn) % Throw warning if number of loaded video files is different from number of signal files
        ...
        function obj = editVideoOffset(obj)
            obj.videoObj.timeOffsetS = str2double(obj.h.eVideoOffset.String);
            obj.videoShowObj = obj.videoShowObj.updateStreams;
        end
        function obj = decreaseVideoOffset(obj)
            obj.h.eVideoOffset.String = obj.videoShowObj.videoTimeOffset;
            obj.videoObj.timeOffsetS = str2double(obj.h.eVideoOffset.String);
            obj.videoShowObj = obj.videoShowObj.updateStreams;
        end
        function obj = increaseVideoOffset(obj)
            obj.h.eVideoOffset.String = obj.videoShowObj.videoTimeOffset;
            obj.videoObj.timeOffsetS = str2double(obj.h.eVideoOffset.String);
            obj.videoShowObj = obj.videoShowObj.updateStreams;
        end
...
kf_ = kf
        ...
        function cbResize(obj, hf, ~)
            obj.h.panNav.Position(2) = hf.Position(4) - obj.h.panNav.Position(4);
...
%             hf.Children(3).Position(2) = hf.Position(4) - 60;
        ...
        function obj = easterEgg03(obj) % Shows a painting by Roger Dean in a new window
            fn = fieldnames(obj.h);
            for kfn = 1 : numel(fn)
                if isprop(obj.h.(fn{kfn}), 'BackgroundColor')
                    if ~all(obj.h.(fn{kfn})(1).BackgroundColor == [0.7 1 1])
                        set(obj.h.(fn{kfn}), 'BackgroundColor', [0.7 1 1])
                    else
                        set(obj.h.(fn{kfn}), 'BackgroundColor', [0.9608    0.9608    0.9608]);
                    end
                end
            end
        end
        function obj = easterEgg04(obj) % Shows a painting by Roger Dean in a new window
            fn = fieldnames(obj.h);
            for kfn = 1 : numel(fn)
                if isprop(obj.h.(fn{kfn}), 'BackgroundColor')
                    if ~all(obj.h.(fn{kfn})(1).BackgroundColor == [1 0.7 1])
                        set(obj.h.(fn{kfn}), 'BackgroundColor', [1 0.7 1])
                    else
                        set(obj.h.(fn{kfn}), 'BackgroundColor', [0.9608    0.9608    0.9608]);
                    end
                end
        ...
        function obj = fileUpdate(obj)
            ...
            obj.h.tSamplingFreq.String = ['Fsₘₐₓ=', num2str(max(obj.signalObj.plotTbl.Fs)), 'Hz'];
            % Load new label
            if ~isempty(obj.labelObj)
        ...
        function obj = createWindow(obj)
            ...
            obj.h.mVideoLoad2 = uimenu(obj.h.mVideo, 'Label', '&Load -3...', 'Callback', @obj.cbMenu,...
                'Tag', 'videoLoad3');
            obj.h.mVideoLoad2 = uimenu(obj.h.mVideo, 'Label', '&Load -4...', 'Callback', @obj.cbMenu,...
                'Tag', 'videoLoad4');
            ...
            % View control
            obj.h.panViewControl = uipanel('Units', 'pixels', 'Position', obj.stg.cPanDisplayCtrPos);
            obj.h.cWholeFile = uicontrol('Parent', obj.h.panViewControl, 'Style', 'checkbox', 'Units', 'pixels', 'HorizontalAlignment', 'right',...
                'Position', [...
                0,...
            ...
            obj.h.cLinkVertZoom = uicontrol('Parent', obj.h.panViewControl, 'Style', 'checkbox', 'Units', 'pixels', 'HorizontalAlignment', 'right',...
                'Position', [...
                0,...
                obj.stg.cPanDisplayCtrPos(4)*1/3,...
                obj.stg.cPanDisplayCtrPos(3)/1,...
            ...
            obj.h.cNeuroSignalStudie = uicontrol('Parent', obj.h.panViewControl, 'Style', 'pushbutton', 'Units', 'pixels', 'HorizontalAlignment', 'right',...
                'Position', [...
                0,...
            ...
            obj.h.tSignalpn = uicontrol('Parent', obj.h.panFilepn, 'Style', 'text', 'Units', 'pixels', 'HorizontalAlignment', 'right',...
                'Position', [...
                obj.stg.cPanFileNumberPos(3)*3,...
                obj.stg.cPanFileNumberPos(4)*2/3,...
                obj.stg.cPanFileNumberPos(3)*0.5,...
                obj.stg.cPanFileNumberPos(4)*1/3],...
                'String', 'Signal:',...
                'Tag', 'tSignalpn');
            obj.h.eSignalpn = uicontrol('Parent', obj.h.panFilepn, 'Style', 'edit', 'Units', 'pixels', 'HorizontalAlignment', 'left',...
                'Position', [...
                obj.stg.cPanFileNumberPos(3)*3.5,...
                obj.stg.cPanFileNumberPos(4)*2/3,...
                obj.stg.cPanFilepnPos(3)*6/10,...
                obj.stg.cPanFileNumberPos(4)*1/3],...
            ...
            obj.h.tCurrentFile = uicontrol('Parent', obj.h.panFilepn, 'Style', 'text', 'Units', 'pixels', 'HorizontalAlignment', 'right',...
                'Position', [...
                0,...
                obj.stg.cPanFileNumberPos(4)*2/3,...
                obj.stg.cPanFileNumberPos(3)/2,...
                obj.stg.cPanFileNumberPos(4)*1/3],...
                'String', 'Current file:', 'Tag', 'testCurrentFile');
            obj.h.eCurrentFile = uicontrol('Parent', obj.h.panFilepn, 'Style', 'edit', 'Units', 'pixels', 'HorizontalAlignment', 'right',...
                'Position', [...
                obj.stg.cPanFileNumberPos(3)/2,...
                obj.stg.cPanFileNumberPos(4)*2/3,...
                obj.stg.cPanFileNumberPos(3)/4,...
                ...
                'String', '0', 'Callback', @obj.cbUicontrol,...
                'Tag', 'editCurrentFile', 'Enable', 'off');
            obj.h.tTotalFiles = uicontrol('Parent', obj.h.panFilepn, 'Style', 'text', 'Units', 'pixels', 'HorizontalAlignment', 'left',...
                'Position', [...
                obj.stg.cPanFileNumberPos(3)*3/4,...
                obj.stg.cPanFileNumberPos(4)*2/3,...
                obj.stg.cPanFileNumberPos(3)/4,...
                ...
                'String', '/0',...
                'Tag', 'textTotalFiles');
            obj.h.tSamplingFreq = uicontrol('Parent', obj.h.panFilepn, 'Style', 'text', 'Units', 'pixels', 'HorizontalAlignment', 'center',...
                'Position', [...
                obj.stg.cPanFileNumberPos(3),...
                obj.stg.cPanFileNumberPos(4)*2/3,...
                obj.stg.cPanFileNumberPos(3)*2,...
                obj.stg.cPanFileNumberPos(4)*1/3],...
                'String', 'Fsₘₐₓ=N/A',...
            ...
            % % %     'Tag', 'eSamplingFreq', ...
            % % %     'Enable', 'on');
            obj.h.tPlaySpeed = uicontrol('Parent', obj.h.panFilepn, 'Style', 'text', 'Units', 'pixels', 'HorizontalAlignment', 'right',...
                'Position', [...
                0,...
                obj.stg.cPanFileNumberPos(4)*1/3,...
                obj.stg.cPanFileNumberPos(3)/2,...
                ...
                'String', 'Play speed:',...
                'Tag', 'tPlaySpeed');
            obj.h.ePlaySpeed = uicontrol('Parent', obj.h.panFilepn, 'Style', 'edit', 'Units', 'pixels', 'HorizontalAlignment', 'right',...
                'Position', [...
                obj.stg.cPanFileNumberPos(3)/2,...
                obj.stg.cPanFileNumberPos(4)*1/3,...
                obj.stg.cPanFileNumberPos(3)/4,...
                ...
                'String', '1', 'Callback', @obj.cbUicontrol,...
                'Tag', 'editPlaySpeed', 'Enable', 'off');
            obj.h.bPlaySpeed(1) = uicontrol('Parent', obj.h.panFilepn, 'Style', 'pushbutton', 'Units', 'pixels',...
                'Position', [...
                obj.stg.cPanFileNumberPos(3)*6/8,...
                obj.stg.cPanFileNumberPos(4)*1/3,...
                obj.stg.cPanFileNumberPos(3)/8,...
                ...
                'Tooltip', 'decreasePlaySpeed',...
                'Tag', 'decreasePlaySpeed', 'Enable', 'off');
            obj.h.bPlaySpeed(2) = uicontrol('Parent', obj.h.panFilepn, 'Style', 'pushbutton', 'Units', 'pixels',...
                'Position', [...
                obj.stg.cPanFileNumberPos(3)*7/8,...
                obj.stg.cPanFileNumberPos(4)*1/3,...
                obj.stg.cPanFileNumberPos(3)/8,...
                ...
                'Tooltip', 'increasePlaySpeed',...
                'Tag', 'increasePlaySpeed', 'Enable', 'off');
            obj.h.tBrightness = uicontrol('Parent', obj.h.panFilepn, 'Style', 'text', 'Units', 'pixels', 'HorizontalAlignment', 'right',...
                'Position', [...
                obj.stg.cPanFileNumberPos(3),...
                obj.stg.cPanFileNumberPos(4)*1/3,...
                obj.stg.cPanFileNumberPos(3)/2,...
                ...
                'String', 'Brightness:',...
                'Tag', 'tBrightness');
            obj.h.eBrightness = uicontrol('Parent', obj.h.panFilepn, 'Style', 'edit', 'Units', 'pixels', 'HorizontalAlignment', 'right',...
                'Position', [...
                obj.stg.cPanFileNumberPos(3)*3/2,...
                obj.stg.cPanFileNumberPos(4)*1/3,...
                obj.stg.cPanFileNumberPos(3)/4,...
                obj.stg.cPanFileNumberPos(4)*1/3],...
                'String', '1', 'Callback', @obj.cbUicontrol,...
                'Tag', 'editBrightness', 'Enable', 'off');
            obj.h.bBrightness(1) = uicontrol('Parent', obj.h.panFilepn, 'Style', 'pushbutton', 'Units', 'pixels',...
                'Position', [...
                obj.stg.cPanFileNumberPos(3)*14/8,...
                obj.stg.cPanFileNumberPos(4)*1/3,...
                obj.stg.cPanFileNumberPos(3)/8,...
                obj.stg.cPanFileNumberPos(4)*1/3],...
                'String', '-',...
                ...
                'Tooltip', 'decreaseBrightness',...
                'Tag', 'decreaseBrightness', 'Enable', 'off');
            obj.h.bBrightness(2) = uicontrol('Parent', obj.h.panFilepn, 'Style', 'pushbutton', 'Units', 'pixels',...
                'Position', [...
                obj.stg.cPanFileNumberPos(3)*15/8,...
                obj.stg.cPanFileNumberPos(4)*1/3,...
                obj.stg.cPanFileNumberPos(3)/8,...
                obj.stg.cPanFileNumberPos(4)*1/3],...
                'String', '+',...
                ...
                'Tooltip', 'increaseBrightness',...
                'Tag', 'increaseBrightness', 'Enable', 'off');
            obj.h.tVideoOffset = uicontrol('Parent', obj.h.panFilepn, 'Style', 'text', 'Units', 'pixels', 'HorizontalAlignment', 'right',...
                'Position', [...
                obj.stg.cPanFileNumberPos(3)*2,...
                obj.stg.cPanFileNumberPos(4)*1/3,...
                obj.stg.cPanFileNumberPos(3)/2,...
                ...
                'String', 'Video offset:',...
                'Tag', 'tVideoOffset');
            obj.h.eVideoOffset = uicontrol('Parent', obj.h.panFilepn, 'Style', 'edit', 'Units', 'pixels', 'HorizontalAlignment', 'right',...
                'Position', [...
                obj.stg.cPanFileNumberPos(3)*5/2,...
                obj.stg.cPanFileNumberPos(4)*1/3,...
                obj.stg.cPanFileNumberPos(3)/4,...
                obj.stg.cPanFileNumberPos(4)*1/3],...
                'String', '0', 'Callback', @obj.cbUicontrol,...
                'Tag', 'editVideoOffset', 'Enable', 'off');
            obj.h.bVideoOffset(1) = uicontrol('Parent', obj.h.panFilepn, 'Style', 'pushbutton', 'Units', 'pixels',...
                'Position', [...
                obj.stg.cPanFileNumberPos(3)*22/8,...
                obj.stg.cPanFileNumberPos(4)*1/3,...
                obj.stg.cPanFileNumberPos(3)/8,...
                obj.stg.cPanFileNumberPos(4)*1/3],...
                'String', '-',...
                'Callback', @obj.cbUicontrol,... % The callback function knows which button was pressed from the Tag
                'Tooltip', 'decreaseVideoOffset',...
                'Tag', 'decreaseVideoOffset', 'Enable', 'off');
            obj.h.bVideoOffset(2) = uicontrol('Parent', obj.h.panFilepn, 'Style', 'pushbutton', 'Units', 'pixels',...
                'Position', [...
                obj.stg.cPanFileNumberPos(3)*23/8,...
                obj.stg.cPanFileNumberPos(4)*1/3,...
                obj.stg.cPanFileNumberPos(3)/8,...
                obj.stg.cPanFileNumberPos(4)*1/3],...
                'String', '+',...
                'Callback', @obj.cbUicontrol,... % The callback function knows which button was pressed from the Tag
                'Tooltip', 'increaseVideoOffset',...
                'Tag', 'increaseVideoOffset', 'Enable', 'off');
            % Startup donkey
