% Convert Intan RHS file to EDF using edfwrite (Signal Processing Toolbox)
% This script converts Intan RHS, MAT, or Spike2 (.smrx) files to EDF format.
% Main features:
% - Supports multiple files via interactive selection.
% - Loads signals dynamically based on file type (.rhs, .mat, .smrx).
% - Reorders channels according to an external channel order file.
% - Filters channels for the selected patient.
% - Converts signals to int16 and scales to microvolts.
% - Sets EDF header dynamically, including start date and time from filename.
% - Allows the user to select the folder to save EDF files.
% - Writes each signal file to EDF format using edfwrite.
...
close all
hdr = edfheader("EDF");
%----- Should be type by the user--------------
hdr.Patient = "JK007295";
%----------------------------------------------
% Get signal file path and names snlpn
basepath = '../../';
pathWorkDir = pwd; % Save current path
startpath = fullfile(pwd, basepath);
cd(startpath); % Go to the startpath
startpath = pwd; % Get the startpath name without the /../
cd(pathWorkDir); % Change to the original current path
basepath = startpath;
prompt = 'Select signal files';
[filen, filep] =  uigetfile({ ...
    '*.smrx;*.mat', 'Spike2 or MAT Files (*.smrx, *.mat)'; ...
    '*.smrx', 'Spike2 Files (*.smrx)'; ...
    '*.mat', 'MAT-files (*.mat)'; ...
    '*.rhs', 'RHS files (*.rhs)'}, ...
    prompt, startpath, 'MultiSelect', 'on');
...
if ~iscell(filepn) % If only one file was found, it will be by default just a character array. Here we place it in a cell array for consistency with the case when multiple files were found.
    c = {filepn};
    filepn = c;
end
snlpn = filepn;
[startpath,~,ext] = fileparts(snlpn{1});
% Where to save new edf files
edfSvPath = uigetdir([pwd, '/../'], 'Where to save the edf files');
if ~exist(edfSvPath, "dir")
    mkdir(edfSvPath)
end

prompt = 'Select channel order file';
[filenChnOr, filepChnOr] = uigetfile(startpath, prompt, 'MultiSelect', 'on');
channelorder = table2array(readtable(fullfile(filepChnOr,filenChnOr)));
for file_indx = 1 : length(snlpn) % file number
    % Step 1: Read file
    if ext == ".mat"
        % Load signal
        load(snlpn{file_indx}); %load sigTbl
    else
        % Load signal
        sigTbl = loadSignal(snlpn{file_indx});
    end
    if isempty(sigTbl); return;end
    recPos_subjName = sigTbl.ChName(contains(sigTbl.ChName , hdr.Patient));
    recPos_subjName = split(recPos_subjName,'-');
    sigTbl = sigTbl(contains(sigTbl.ChName , strjoin([recPos_subjName{1},"-"],"")),:); 
    sigTbl = sigTbl(channelorder,:);
    signalData = int16(cell2mat(sigTbl.Data)./0.195);
    % Step 2: Prepare timetable for edfwrite
    num_channels = size(signalData, 1);
    num_samples  = size(signalData, 2);
    hdr.NumSignals = num_channels;
    hdr.NumDataRecords = 1;
    hdr.PhysicalMin = -5*ones(1, num_channels);
    hdr.PhysicalMax = 5*ones(1, num_channels);
    hdr.DigitalMin = -32768*ones(1, num_channels);
    hdr.DigitalMax = 32767*ones(1, num_channels);
    [~, fn] = fileparts(snlpn{file_indx});
    fileNameParts = split(fn,'_');
    dt = datetime([fileNameParts{2} fileNameParts{3}], "InputFormat", "yyMMddHHmmss");
    hdr.StartDate = datestr(dt, 'dd.mm.yy');
    hdr.StartTime = datestr(dt, 'HH.MM.SS');
    edf_filename = [fn '.edf'];
    edfwrite([edfSvPath,'\',edf_filename], hdr, signalData')
    % edfinfo("NLU_250827_105345.edf")
    disp(['EDF file written: ', edf_filename]);
end
...
% % % 
% % % % Build timetable with channel data
% % % TT = array2timetable(signalData', 'RowTimes', seconds(t));
% % % 
% % % % Assign channel names
% % % labels = cell(1, num_channels);
% % % for ch = 1:num_channels
% % %     labels{ch} = sprintf('Chan%d', ch);
% % % end
% % % TT.Properties.VariableNames = channelNames;
    ...
    if isfield(l, 'subject')
        if isstring(l.subject)
            if length(l.subject) == size(l.s, 1)
                Subject = l.subject(:);
            elseif length(l.subject) == 1 %#ok<ISCL>
...
function sigTbl = loadCVUT(l)
    [numSamp, numCh] = size(l.d);
    if l.fs - 1/(mean(diff(l.tabs))*3600*24) > 10
        warning('_jk l.fs ~= 1/(mode(diff(tabs))*3600*24)')
        pause
    end
    assignin('base', 'tabs', l.tabs)
    if numSamp ~= size(l.tabs, 1)
        warning('_jk size(l.d, 1) ~= size(l.tabs, 1)')
        pause
    end
    Subject = repelem("Unknown", numCh)';
    ChName = string(l.header.label);
    SigStart = datetime(ones(numCh, 1) * l.tabs(1), 'ConvertFrom', 'datenum');
    SigEnd = SigStart + seconds(numSamp/l.fs);
    Fs = ones(numCh, 1) * l.fs;
    for kch = 1 : numCh
        Data{kch, 1} = (single(l.d(:, kch))*single(l.mults(kch)))'; %#ok<AGROW>
    end
...
function sigTbl = loadSmrx(filepn)
    ...
    [filep, filen, ~] = fileparts(filepn); %#ok<ASGLU>
    ...
    if (fhand <= 0);  warning(['_jk Could not load ', filepn]); CEDS64ErrorMessage(fhand); unloadlibrary ceds64int;sigTbl = []; return; end
