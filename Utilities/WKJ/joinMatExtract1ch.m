...
kN = k;
    ...
    [l.s, l.fs] = res2fs5000(l.s(ch, :), l.fs, l.intScale, l.intOffset); % Resample to 5000 if needed
    ...
    while l.numSamp/l.fs < finalDur && k < numFiles
        ...
        [sadd, ~] = res2fs5000(moadd.s(ch, :), moadd.fs, moadd.intScale, moadd.intOffset); % Resample to 5000 if needed
        l.numSamp = l.numSamp + size(sadd, 2);
size(sadd)
        l.s = [l.s, sadd];
    end
    l.numChan = 1;
    l.chanNames = l.chanNames(ch);
