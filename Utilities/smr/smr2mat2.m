...
fsDesired = 5000; % Desired fs of output files

addpath ..\son

% Get smr-files
[filepn, filep, filen] = getFilepnAllCell('Select smr-files', 'smr');
...
disp(smrfname)
    ...
    % Decimate if needed
    if fsDesired < allVar.fs
        allVar.s = int16Decim(allVar.s, allVar.fs, fsDesired);
        allVar.fs = fsDesired;
    end
...
disp([10 'Conversion finished. ' num2str(numFilesConverted)...
    ' files were converted.' 10 'Source smr-files are in folder ' filep 10 ...
...
%% intDecim
function s = int16Decim(s, fs, fsd)
dF = fs/fsd;
if dF ~= 1
disp('Resampling')
    s = double(s);
