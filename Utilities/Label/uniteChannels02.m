...
filepn{kf}
    ...
    for klbl = 1 : length(lblnm)
        ...
        for kch = 1 : length(chnm)
            ...
            %% Make row
            finm = fieldnames(label.(lblnm{klbl}).(chnm{kch}));
            for ktr = 1 : length(finm)
                if isnumeric(label.(lblnm{klbl}).(chnm{kch}).(finm{ktr}))
                    label.(lblnm{klbl}).(chnm{kch}).(finm{ktr}) = label.(lblnm{klbl}).(chnm{kch}).(finm{ktr})(:)';
                end
            end
...
% % % if strcmpi(lblnm{klbl}, 'SEIZURE')
% % %     close all
% % %     hax = axes;
% % %     plot(sign(cumsum(updown)))
% % %     hax.YLim = [0 4];
% % %     pause
% % % end
