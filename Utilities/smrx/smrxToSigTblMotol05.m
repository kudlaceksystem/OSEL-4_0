%% Use this for the new mice which are registered in SoftMouse.net and which have the full name in the channel name (e.g. L-A-SK000917)
function smrxToSigTblMotol05
...
function [sigT, dateS, subjNm] = loadSmrx(filepn)
    ...
    function loadPrahaMotolChronic
        ...
        for kch2 = 1 : length(chnm)
            ...
            computers = {'Vochomurka', 'Ferda', 'Bobik', 'Amalka', 'Myspulin', 'PD', 'PS', 'NRD', 'NRM', 'NRU'};
...
%             SigStart = datetime(fliplr(td(2 : end)));
        ...
        % Change subject names if necessary
        for kch2 = 1 : size(sigTbl, 1)
            ...
            % % % % % % % %     subj = ['SK', subj];
            if ~isempty(regexp(subj, '\D\D\d\d\d\d\d\d', 'once'))
                subj = subj;
            else
                subjectt = subj;
                error(['_jk Unknown owner of mouse ', subjectt])
            end
