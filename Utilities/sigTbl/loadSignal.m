...
pause
    ...
    function loadPrahaMotolChronic
        ...
        disp(['Loading channel ', num2str(0, '%03.0f'), '/', num2str(length(chnm), '%03.0f')])
        for kch2 = 1 : length(chnm)
            fprintf(repmat('\b', 1, 24));
            disp(['Loading channel ', num2str(kch2, '%03.0f'), '/', num2str(length(chnm), '%03.0f')])
            str = chnm{kch2};
...
%                 Subject = 'BranoTheRat';
            ...
            % % % % % dt = r{1};
            % % % % % SigStart = datetime(dt, 'InputFormat', 'yyMMdd_HHmmss');
            r = regexp(filen, '\d+ \d+ \d+_\d+ \d+ \d+', 'match');
            dt = r{1};
            SigStart = datetime(dt, 'InputFormat', 'dd MM yy_HH mm ss', 'Format', 'yyMMdd_HHmmss'); % Extract the signal datetime
            SigStart = SigStart - hours(2);
            % [~, td]  = CEDS64TimeDate(fhand);
            % smrxInfoDt = datetime([td(7), td(6), td(5), td(4), td(3), td(2)]);
            % timeDif = SigStart - smrxInfoDt;
            % SigStart = SigStart - round(timeDif, "hours");
