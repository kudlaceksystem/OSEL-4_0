...
Info=smr.SONChannelInfo(fid,chan);
...
FileH=smr.SONFileHeader(fid);
SizeOfHeader=20;                                            % Block header is 20 bytes long
header=smr.SONGetBlockHeaders(fid,chan);
...
h.title=Info.title;
h.sampleinterval=smr.SONGetSampleInterval(fid,chan);
h.scale=Info.scale;
...
[h.start,h.TimeUnits]=smr.SONTicksToSeconds(fid,h.start,varargin{:});
[h.stop,h.TimeUnits]=smr.SONTicksToSeconds(fid,h.stop,varargin{:});

if ScaleData==1
    if ShowProgress==1
        progressbar(1,progbar,'Scaling data.....');
    end;
    [data,h]=smr.SONADCToDouble(data,h);
end;
