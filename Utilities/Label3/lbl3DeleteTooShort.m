function lbl3DeleteTooShort
% Deletes all labels of speicified class shorter than specified duration.
...
for kf = 1 : length(filepn)
    ...
    indLblClass = lblSet.ClassName == lblClassName;
    indTooShort = (lblSet.End - lblSet.Start) < seconds(minLblDurS);
    lblSet = lblSet(~(indLblClass & indTooShort), :);
    if isempty(lblSet)
        continue
    end
