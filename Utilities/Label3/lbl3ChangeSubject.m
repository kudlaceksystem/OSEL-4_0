function lbl3ChangeSubject
    ...
    for kf = 1 : length(filepn)
        ...
        sigInfo.Subject(sigInfo.Subject == oldSubject) = newSubject;

        %% Save new lbl3 file
        oldFilen = filen{kf};
        ss = strsplit(oldFilen, '-');
        % if oldSubject ~= string(ss{1})
        %     error(['_jk oldSubject: ', char(oldSubject), ', file name: ', ss{1}])
        % end
